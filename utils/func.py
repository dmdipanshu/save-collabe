...
def E(L):   
    private_match = re.match(r'https?://(?:t\.me|telegram\.me)/c/(\d+)/(?:\d+/)?(\d+)', L)
    public_match = re.match(r'https?://(?:t\.me|telegram\.me)/([^/]+)/(?:\d+/)?(\d+)', L)
...
_video_metadata_executor = concurrent.futures.ThreadPoolExecutor(max_workers=4)
...
async def get_video_metadata(file_path):
    ...
    try:
        ...
        return await loop.run_in_executor(_video_metadata_executor, _extract_metadata)
