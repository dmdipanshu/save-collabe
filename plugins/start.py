...
async def subscribe(app, message):
    if FORCE_SUB and FORCE_SUB != -10012345567:
        try:
            ...
            try:
                ...
                print(f"Error exporting invite link for FORCE_SUB: {e}")
                return 0
        except Exception as ggn:
            error_str = str(ggn).upper()
            if "CHAT_ID_INVALID" in error_str or "PEER_ID_INVALID" in error_str or "CHANNEL_INVALID" in error_str or "NOT_FOUND" in error_str:
                print(f"WARNING: FORCE_SUB channel ({FORCE_SUB}) is invalid, unknown, or inaccessible to the bot. Skipping force subscribe check.")
                return 0
...
async def send_or_edit_help_page(_, message, page_number):
    ...
    if getattr(message, "edit_text", None) and getattr(message, "from_user", None) and getattr(message.from_user, "is_bot", False):
        try:
